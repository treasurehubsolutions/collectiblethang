  ...
  return (
    <>
      ...
      <header style={{background:'#07070f',borderBottom:'1px solid #1c1c20',position:'sticky',top:0,zIndex:100}}>
        <div style={{maxWidth:1300,margin:'0 auto',padding:'0 16px',display:'flex',alignItems:'center',gap:12,height:60}}>

          {/* Logo */}
          <Link href="/" style={{textDecoration:'none',flexShrink:0}}>
            <img src="/logo.jpg" alt="THE SHELF CARTEL" style={{height:44,width:'auto',objectFit:'contain'}}/>
          </Link>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8,marginLeft:'auto',flexShrink:0}}>
            <button onClick={()=>setLang(lang==='fr'?'en':'fr')}
              style={{background:'#12121e',border:'1px solid #2a2a20',borderRadius:4,padding:'5px 10px',color:'#888',fontSize:11,fontWeight:700,cursor:'pointer'}}>
              {lang==='fr'?'EN':'FR'}
            </button>
        ...
        {/* Category nav */}
        <div style={{background:'#0a0a10',borderTop:'1px solid #1c1c20',overflowX:'auto'}}>
          <div style={{maxWidth:1300,margin:'0 auto',padding:'0 16px',display:'flex',gap:0}}>
            {CATS.map(cat=>(
              <Link key={cat} href={cat==='Deals'?'/shop?sort=price_asc':`/shop?category=${encodeURIComponent(cat)}`}
                style={{padding:'8px 14px',fontSize:12,color:cat==='Deals'?'#c9a227':'#888',textDecoration:'none',whiteSpace:'nowrap',borderBottom:'2px solid transparent',fontWeight:cat==='Deals'?700:400}}>
                {cat}
              </Link>
            ))}
          </div>
