...
const CATEGORIES = [
  'Hot Wheels','Hot Wheels Premium','Star Wars','Marvel','DC Comics','Transformers',
  'Jurassic Park / World','WWE & Wrestling','G.I. Joe','TMNT','Power Rangers',
  'Masters of the Universe','Sonic the Hedgehog','Pokémon','Hallmark Ornaments',
  'Funko Pop','McFarlane Figures','Disney & Pixar','LEGO','VHS Tapes','Video Games',
  'Video Game Figures','Diecast & Scale Models','Action Figures','Little People Collector',
  'Dolls & Barbie','Hockey','Playmobil','Fortnite','Électroménager','Bar & Brasserie','Autres'
]
...
export default function AdminPage() {
  ...
  if (!auth) return (
    <div style={{maxWidth:380,margin:'120px auto',padding:'0 24px',textAlign:'center'}}>
      ...
      <button onClick={login} style={{width:'100%',background:'#e8b820',color:'#000',border:'none',borderRadius:8,padding:'13px',fontWeight:800,fontSize:15,cursor:'pointer'}}>
        Se connecter
      </button>
    </div>
  )
    ...
    <div style={{marginBottom:14}}>
      ...
      {options
        ? <select value={form[k]} onChange={e=>setForm(f=>({...f,[k]:e.target.value}))}
            style={{width:'100%',padding:'9px 12px',borderRadius:6,background:'#12121e',border:'1px solid #1c1c28',color:'#eee',fontSize:13,outline:'none'}}>
    ...
    <div style={{maxWidth:1300,margin:'0 auto',padding:'24px'}}>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24,flexWrap:'wrap',gap:12}}>
        ...
        <div style={{display:'flex',gap:8}}>
          <button onClick={()=>{setForm(EMPTY);setEditId(null);setTab('add')}}
            style={{background:'#e8b820',color:'#000',border:'none',borderRadius:6,padding:'9px 18px',fontWeight:800,fontSize:13,cursor:'pointer'}}>
            + Ajouter un produit
          </button>
      ...
      <div style={{display:'flex',gap:0,marginBottom:24,borderBottom:'1px solid #1c1c28'}}>
        {[['products',`Produits (${products.length})`],['add',editId?'Modifier':'Ajouter']].map(([t,l])=>(
          <button key={t} onClick={()=>setTab(t)}
            style={{padding:'10px 20px',background:'none',border:'none',borderBottom:tab===t?'2px solid #e8b820':'2px solid transparent',color:tab===t?'#e8b820':'#666',fontWeight:tab===t?700:400,fontSize:14,cursor:'pointer'}}>
            {l}
          </button>
        ))}
      </div>
        ...
        <div>
          <div style={{display:'flex',gap:10,marginBottom:14,flexWrap:'wrap'}}>
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Rechercher..."
              style={{flex:1,minWidth:200,padding:'8px 14px',background:'#12121e',border:'1px solid #1c1c28',borderRadius:6,color:'#eee',fontSize:13,outline:'none'}}/>
            {[['all','Tous'],['enabled','Actifs'],['disabled','Masqués']].map(([v,l])=>(
              <button key={v} onClick={()=>setFilter(v)}
                style={{padding:'8px 14px',borderRadius:6,border:'1px solid #1c1c28',background:filter===v?'#e8b820':'#12121e',color:filter===v?'#000':'#888',fontWeight:filter===v?700:400,fontSize:12,cursor:'pointer'}}>
                {l}
              </button>
            ))}
          </div>
          <div style={{background:'rgba(232,184,32,.07)',border:'1px solid rgba(232,184,32,.15)',borderRadius:8,padding:'10px 14px',marginBottom:14,fontSize:12,color:'#999'}}>
            💡 Clique <strong style={{color:'#e8b820'}}>Actif / Masqué</strong> pour changer la visibilité · <strong style={{color:'#e8b820'}}>Modifier</strong> pour éditer · <strong style={{color:'#e05050'}}>Supprimer</strong> pour effacer définitivement
          </div>
                ...
                <div key={p.id} style={{display:'flex',alignItems:'center',gap:12,background:p.enabled?'#12121e':'#0a0a10',border:`1px solid ${p.enabled?'#1c1c28':'#2a1010'}`,borderRadius:8,padding:'10px 14px',opacity:p.enabled?1:.6}}>
                  ...
                  <div style={{display:'flex',gap:6,flexShrink:0}}>
                    <button onClick={()=>toggleProduct(p.id,p.enabled)}
                      style={{padding:'5px 12px',borderRadius:5,border:'none',background:p.enabled?'rgba(74,222,128,.12)':'rgba(248,113,113,.12)',color:p.enabled?'#4ade80':'#f87171',fontWeight:700,fontSize:11,cursor:'pointer'}}>
                      {p.enabled?'✓ Actif':'✗ Masqué'}
                    </button>
                    <button onClick={()=>editProduct(p)}
                      style={{padding:'5px 12px',borderRadius:5,border:'1px solid #2a2a38',background:'none',color:'#aaa',fontWeight:600,fontSize:11,cursor:'pointer'}}>
                      ✏️ Modifier
                    </button>
                    <button onClick={()=>deleteProduct(p.id)}
                      style={{padding:'5px 10px',borderRadius:5,border:'none',background:'rgba(220,38,38,.1)',color:'#f87171',fontWeight:700,fontSize:11,cursor:'pointer'}}>
                      🗑️
                    </button>
                  </div>
                </div>
      ...
      {tab==='add' && (
        <div style={{maxWidth:700}}>
          <h2 style={{fontFamily:'Bebas Neue',fontSize:22,letterSpacing:2,marginBottom:20,color:'#e8b820'}}>
          ...
          <F label="Poids (grammes, pour calcul livraison)" k="weight" type="number"/>
          <div style={{marginBottom:14}}>
            ...
            <textarea value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} rows={4}
              style={{width:'100%',padding:'9px 12px',borderRadius:6,background:'#12121e',border:'1px solid #1c1c28',color:'#eee',fontSize:13,outline:'none',resize:'vertical'}}/>
          </div>
              ...
              {form.photos.map((ph,i)=>(
                <div key={i} style={{position:'relative',width:80,height:80,borderRadius:8,overflow:'hidden',border:'1px solid #1c1c28',background:'#0d0d12'}}>
                  ...
                  <button onClick={()=>setForm(f=>({...f,photos:f.photos.filter((_,j)=>j!==i)}))}
                    style={{position:'absolute',top:2,right:2,width:18,height:18,borderRadius:'50%',background:'rgba(220,38,38,.9)',color:'#fff',border:'none',cursor:'pointer',fontSize:10,display:'flex',alignItems:'center',justifyContent:'center',fontWeight:700}}>✕</button>
                </div>
              ))}
              <button onClick={()=>fileRef.current.click()}
                style={{width:80,height:80,borderRadius:8,border:'2px dashed #2a2a38',background:'#12121e',color:'#666',cursor:'pointer',fontSize:24,display:'flex',alignItems:'center',justifyContent:'center'}}>
                {uploading?'⏳':'+'}
              </button>
            </div>
            <input ref={fileRef} type="file" accept="image/*" multiple style={{display:'none'}}
              onChange={async e=>{for(const file of e.target.files) await uploadPhoto(file); e.target.value=''}}/>
            <div style={{fontSize:11,color:'#555'}}>Formats acceptés: JPG, PNG, WEBP · Max 5MB par photo</div>
          </div>

          <div style={{display:'flex',gap:10}}>
            <button onClick={saveProduct} disabled={saving}
              style={{flex:1,background:'#e8b820',color:'#000',border:'none',borderRadius:8,padding:'14px',fontWeight:800,fontSize:15,cursor:'pointer',opacity:saving?.7:1}}>
              {saving?'Sauvegarde...':(editId?'💾 Mettre à jour':'✓ Ajouter le produit')}
            </button>
            <button onClick={()=>{setForm(EMPTY);setEditId(null);setTab('products')}}
              style={{padding:'14px 20px',background:'#12121e',color:'#888',border:'1px solid #1c1c28',borderRadius:8,fontWeight:600,fontSize:14,cursor:'pointer'}}>
              Annuler
            </button>
          </div>
        </div>
