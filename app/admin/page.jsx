...
export default function AdminPage() {
  ...
  async function bulkDelete() {
    ...
    await Promise.all([...selected].map(id =>
      fetch('/api/admin', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action:'delete', id }) })
    ))
  ...
  async function bulkToggle(enable) {
    ...
    setBulkLoading(true)
    await Promise.all([...selected].map(id =>
      fetch('/api/admin', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action:'toggle', id, enabled: enable }) })
    ))
    setBulkLoading(false)
    setMsg(`✓ ${selected.size} produit(s) ${enable ? 'activé(s)' : 'masqué(s)'}`)
    setTimeout(() => setMsg(''), 3000)
    loadProducts()
  }
    ...
    <div style={{maxWidth:1300,margin:'0 auto',padding:'24px'}}>
      ...
      {tab==='products' && (
        <div>
          ...
          {/* Bulk action bar */}
          <div style={{display:'flex',alignItems:'center',gap:10,padding:'10px 14px',background:'#12121e',border:'1px solid #1c1c28',borderRadius:8,marginBottom:12,flexWrap:'wrap'}}>
            {/* Select all checkbox */}
            <label style={{display:'flex',alignItems:'center',gap:8,cursor:'pointer',userSelect:'none'}}>
              <input type="checkbox" checked={allSelected} onChange={selectAll}
                style={{width:16,height:16,cursor:'pointer',accentColor:'#e8b820'}}/>
              <span style={{fontSize:12,color:'#aaa',fontWeight:600}}>
                {allSelected ? 'Désélectionner tout' : `Tout sélectionner (${filtered.length})`}
              </span>
            </label>
            ...
            {someSelected && (
              <>
                ...
                <span style={{fontSize:12,color:'#e8b820',fontWeight:700}}>{selected.size} sélectionné(s)</span>
                <button onClick={()=>bulkToggle(true)} disabled={bulkLoading}
                  style={{padding:'6px 12px',borderRadius:5,border:'none',background:'rgba(74,222,128,.12)',color:'#4ade80',fontWeight:700,fontSize:11,cursor:'pointer'}}>
                  ✓ Activer tout
                </button>
                <button onClick={()=>bulkToggle(false)} disabled={bulkLoading}
                  style={{padding:'6px 12px',borderRadius:5,border:'none',background:'rgba(248,113,113,.12)',color:'#f87171',fontWeight:700,fontSize:11,cursor:'pointer'}}>
                  ✗ Masquer tout
                </button>
                <button onClick={bulkDelete} disabled={bulkLoading}
                  style={{padding:'6px 12px',borderRadius:5,border:'none',background:'rgba(220,38,38,.15)',color:'#f87171',fontWeight:700,fontSize:11,cursor:'pointer'}}>
                  🗑️ Supprimer tout
                </button>
                {bulkLoading && <span style={{fontSize:11,color:'#555'}}>Traitement...</span>}
              </>
            )}
          </div>
          ...
          {loading ? <div style={{textAlign:'center',padding:60,color:'#555'}}>Chargement...</div> : (
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              {filtered.map(p=>(
                <div key={p.id} style={{display:'flex',alignItems:'center',gap:12,background:selected.has(p.id)?'rgba(232,184,32,.05)':(p.enabled?'#12121e':'#0a0a10'),border:`1px solid ${selected.has(p.id)?'rgba(232,184,32,.3)':(p.enabled?'#1c1c28':'#2a1010')}`,borderRadius:8,padding:'10px 14px',opacity:p.enabled?1:.65}}>
                  {/* Checkbox */}
                  <input type="checkbox" checked={selected.has(p.id)} onChange={()=>toggleSelect(p.id)}
                    style={{width:16,height:16,cursor:'pointer',accentColor:'#e8b820',flexShrink:0}}/>
                  {/* Photo */}
        ...
        <div style={{maxWidth:700}}>
          ...
          <div style={{display:'flex',gap:10}}>
            ...
            <button onClick={()=>{setForm(EMPTY);setEditId(null);setTab('products')}}
              style={{padding:'14px 20px',background:'#12121e',color:'#888',border:'1px solid #1c1c28',borderRadius:8,fontWeight:600,fontSize:14,cursor:'pointer'}}>Annuler</button>
          </div>
        </div>
      )}
    </div>
