...
export default async function ProductPage({ params }) {
  ...
  const { items: related } = await getProducts({ category: p.category, perPage: 6 })
  ...
  return (
    <div style={{maxWidth:1300,margin:'0 auto',padding:'24px'}}>
      <div style={{fontSize:12,color:'#555',marginBottom:20,display:'flex',alignItems:'center',gap:6,flexWrap:'wrap'}}>
        <Link href="/" style={{color:'#666',textDecoration:'none'}}>Accueil</Link><span>›</span>
        <Link href="/shop" style={{color:'#666',textDecoration:'none'}}>Boutique</Link><span>›</span>
      ...
      {others.length>0&&(
        <div style={{marginTop:48}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(170px,1fr))',gap:12}}>
            {others.map(r=>(
              <Link key={r.id} href={`/product/${r.id}`} style={{background:'#12121e',border:'1px solid #1c1c28',borderRadius:8,overflow:'hidden',textDecoration:'none'}}>
